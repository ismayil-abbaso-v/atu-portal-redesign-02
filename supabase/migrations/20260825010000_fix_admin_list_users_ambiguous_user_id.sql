-- Fix admin_list_users: the RETURNS TABLE output column user_id conflicted
-- with the user_roles subquery column named user_id in PL/pgSQL.
-- This caused the admin Users page to fail with:
...
BEGIN
  ...
  WITH filtrli AS (
    ...
    LEFT JOIN (
      SELECT ur.user_id AS role_user_id, array_agg(ur.role) AS rollar
      FROM public.user_roles AS ur
      GROUP BY ur.user_id
    ) AS r ON r.role_user_id = p.user_id
    WHERE
