...
assert(
  JSON.stringify(actualUserRouteFiles) === JSON.stringify(expectedFiles),
  `User-facing route file parity changed from audited source ${SOURCE_HEAD}.\nExpected: ${expectedFiles.join(", ")}\nActual: ${actualUserRouteFiles.join(", ")}`,
);

for (const path of expectedFiles) {
  assert(existsSync(join(root, path)), `Missing user-facing route file: ${path}`);
}
for (const path of [
  "src/routes/api/public/auth-username-login.ts",
  "src/routes/api/public/receive-exam-result.ts",
]) {
...
for (const url of expectedUrls) {
  assert(routeTree.includes("'" + url + "'"), `Generated route tree is missing URL: ${url}`);
}
...
const dynamicChecks = [
  ...
  [
    "src/routes/_authenticated/muellim_.$groupId.$courseId.tsx",
    ["canAccessTeacherCourse", "TeacherCourseWorkspace"],
  ],
...
for (const [path, signals] of dynamicChecks) {
  ...
  for (const signal of signals) {
    assert(content.includes(signal), `Dynamic route ${path} lacks ${signal} handling`);
  }
}
const teacher = readFileSync(
  join(root, "src/components/teacher/TeacherCourseWorkspace.tsx"),
  "utf8",
);
assert(
  teacher.includes("isError || !data"),
  "Teacher dynamic workspace lacks explicit query error state",
);
...
for (const path of [
  ...
  "scripts/prompt11-static-qa.mjs",
]) {
...
console.log(
  `Prompt 12 route completeness passed: ${expectedUrls.length} user-facing URLs, exact source route-file parity, dynamic/error/access/mobile/design coverage verified.`,
);
