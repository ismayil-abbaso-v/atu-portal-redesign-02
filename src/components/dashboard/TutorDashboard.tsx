              ...
              {nearestExams.map((exam) => {
                ...
                return (
                  <button key={exam.id} type="button" onClick={() => void navigate({ to: "/imtahanlar" })} className="tutor-home-list-row flex min-h-[72px] w-full items-center gap-3 rounded-2xl border border-border/70 bg-background px-3 py-3 text-left sm:px-4">
                    <span className="flex min-w-[68px] shrink-0 flex-col items-center justify-center rounded-xl bg-primary/[0.07] px-2 py-2 text-center"><strong className="text-xs font-black text-primary">{formatExamDate(exam.imtahan_tarixi)}</strong><span className="mt-0.5 text-[11px] font-bold uppercase tracking-wide text-muted-foreground">{dayLabel}</span></span>
