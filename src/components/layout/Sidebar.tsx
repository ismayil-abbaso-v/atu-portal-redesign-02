...
function BildirisIkonu({ item, unreadCount }: { item: NavItem; unreadCount: number }) {
  if (item.to !== "/bildirisler" || unreadCount <= 0)
    return <item.icon className="size-[var(--sb-icon-svg)] shrink-0" />;
  return (
...
function PanelEtiketi({ acıq, children }: { acıq: boolean; children: ReactNode }) {
  return (
    <span
      className={cn(
        "pointer-events-none absolute left-[var(--sb-label-left)] top-0 flex h-[var(--sb-icon)] min-w-0 items-center overflow-hidden whitespace-nowrap transition-[max-width,opacity] duration-200 ease-out",
        acıq ? "max-w-[170px] opacity-100" : "max-w-0 opacity-0",
      )}
    >
      <span className="block truncate pl-2 text-[length:var(--sb-text)] font-semibold leading-none">
        {children}
      </span>
    </span>
  );
...
function IkonQutusu({ children }: { children: ReactNode }) {
  return (
    <span className="absolute left-[var(--sb-icon-left)] top-0 flex size-[var(--sb-icon)] shrink-0 items-center justify-center">
      {children}
    </span>
  );
}
...
export function Sidebar({
  ...
  acıq: boolean;
  setAcıq: Dispatch<SetStateAction<boolean>>;
}) {
  ...
  if (primaryTutor)
    dinamikUstBolme.splice(1, 0, {
      to: "/tyutor-paneli",
      labelKey: "tutor.panel",
      icon: LayoutDashboard,
    });

  const dinamikOrtaBolme = [...ortaBolme];
  if (hasDekan)
    dinamikOrtaBolme.push({
      to: "/fakulte-icmali",
      labelKey: "nav.facultyOverview",
      icon: BarChart3,
    });
  if (hasAdmin || hasDekan || primaryTutor)
    dinamikOrtaBolme.push({ to: "/qruplar", labelKey: "nav.groups", icon: Users2 });
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape") setAcıq(false);
    };
  ...
  const link = (item: NavItem) => {
    ...
    return (
      ...
      >
        <IkonQutusu>
          <BildirisIkonu item={item} unreadCount={unreadCount} />
        </IkonQutusu>
        <PanelEtiketi acıq>{label}</PanelEtiketi>
      </Link>
    );
  };
  ...
  const drawerLink = (item: NavItem) => {
    ...
    return (
      ...
      >
        ...
        <span
          className={cn(
            "size-1.5 rounded-full bg-primary transition-all",
            aktiv ? "scale-100 opacity-100" : "scale-0 opacity-0",
          )}
        />
      </Link>
    );
      ...
      <IkonQutusu>
        <span className="flex size-[var(--sb-avatar)] shrink-0 items-center justify-center overflow-hidden rounded-full bg-primary/10 text-primary">
          {avatarUrl ? (
            <SignedAvatarImg
              src={avatarUrl}
              alt={istifadeciAdi || t("nav.profile")}
              className="size-full object-cover"
            />
          ) : (
            <User className="size-[var(--sb-icon-svg)]" />
          )}
        </span>
      </IkonQutusu>
      <PanelEtiketi acıq>{istifadeciAdi}</PanelEtiketi>
    </Link>
  );
    ...
    <>
      ...
      >
        <button
          ...
          onClick={() => setAcıq(false)}
          className={cn(
            "absolute inset-0 bg-foreground/35 backdrop-blur-[6px] transition-opacity duration-300 ease-out",
            acıq ? "opacity-100" : "opacity-0",
          )}
        />
        <aside
        ...
        >
          <div className="portal-mobile-drawer__header flex h-[68px] shrink-0 items-center justify-between border-b border-border/70 px-3">
            ...
            >
              <span className="flex size-10 shrink-0 items-center justify-center overflow-hidden rounded-full bg-primary p-1.5 shadow-sm">
                <img src={atuLogo} alt="ATU" className="size-full object-contain" />
              </span>
            ...
            <button
              ...
              aria-label={t("common.back")}
              onClick={() => setAcıq(false)}
              className="inline-flex size-10 shrink-0 items-center justify-center rounded-xl border border-border bg-card text-muted-foreground transition-[background-color,color,transform] duration-200 hover:bg-accent hover:text-primary active:scale-95"
            >
          ...
          <div className="portal-mobile-drawer__footer shrink-0 border-t border-border/70 p-3">
            <Link
              to="/menyu"
              onClick={() => setAcıq(false)}
              className="flex min-h-14 items-center gap-3 rounded-2xl bg-muted/45 px-3 py-2"
            >
            ...
            <button
              type="button"
              onClick={cıxıs}
              className="mt-2 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl text-sm font-bold text-destructive transition-colors hover:bg-destructive/10"
            >
