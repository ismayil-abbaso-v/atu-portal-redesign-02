...
function ScoreMetric({
  ...
  return (
    <div
      className={`group relative min-w-0 overflow-hidden rounded-xl border px-2 py-2.5 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md min-[380px]:px-2.5 sm:rounded-2xl sm:p-4 ${toneClasses}`}
    >
      ...
      <div className="relative mt-1.5 flex min-h-9 min-w-0 items-end sm:mt-3 sm:min-h-10">
        ...
        {value !== null ? (
          <span className="mb-0.5 ml-1 hidden text-[10px] font-bold text-muted-foreground sm:inline sm:text-xs">
            {jt("common.scoreUnit")}
          </span>
        ) : null}
      </div>
  ...
  return (
    <div className="rounded-xl border border-border/60 bg-background/80 p-3 transition-colors hover:bg-muted/30">
      <div className="mb-2 flex items-center justify-between gap-3">
        ...
        <span className="font-data text-xs font-extrabold tabular-nums text-foreground sm:text-sm">
          {piece.value}/{piece.max}
        </span>
      </div>
...
function AttendanceBadge({ status }: { status: LessonRecordRow["attendance_status"] }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[11px] font-extrabold transition-all duration-300 ${present ? "border-emerald-500/20 bg-emerald-500/10 text-emerald-700 dark:text-emerald-400" : "border-destructive/20 bg-destructive/10 text-destructive"}`}
    >
...
function FileButton({ path, compact = false }: { path: string | null; compact?: boolean }) {
  ...
  return (
    <Button
      ...
      variant="outline"
      size="sm"
      className={`rounded-xl font-bold ${compact ? "h-8 px-2.5 text-xs" : ""}`}
      onClick={() => void openSubmissionFile(path, jt("common.fileOpenError"))}
    >
...
export function StudentJournalView({ userId }: { userId: string }) {
  ...
  const { data: journalData, isLoading: lessonsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (recordRows.length === 0)
        return { entries: [] as JournalEntry[], records: [], sessions: [] as SessionRow[] };
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "course_lesson_sessions" },
        refresh,
      )
      .on(
        "postgres_changes",
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "exam_scores", filter: `user_id=eq.${userId}` },
        refresh,
      )
      .on(
        "postgres_changes",
        {
          event: "*",
          schema: "public",
          table: "independent_work_assessments",
          filter: `student_id=eq.${userId}`,
        },
        refresh,
      )
  ...
  const attended = entries.filter(
    (entry) => entry.record.attendance_status === "iştirak edib",
  ).length;
  ...
  const attendanceStatus: "none" | "risk" | "normal" =
    entries.length === 0
      ? "none"
      : breakdown &&
          breakdown.attendance.absence_limit > 0 &&
          absent >= breakdown.attendance.absence_limit
        ? "risk"
        : "normal";
  ...
  return (
    <div className="animate-page-enter space-y-3 pb-3 sm:space-y-4">
      <Collapsible
        ...
        className="w-full min-w-0 max-w-full overflow-hidden rounded-2xl border border-primary/15 bg-card shadow-sm"
      >
        <div className="relative overflow-hidden bg-primary text-primary-foreground">
          ...
          <div className="relative flex min-w-0 items-center gap-2.5 p-4 sm:gap-3 sm:p-5">
            <CollapsibleTrigger asChild>
              <button
                type="button"
                className="group flex min-w-0 flex-1 items-center gap-3 rounded-xl text-left focus-visible:outline-primary-foreground"
              >
                ...
                <span className="min-w-0 flex-1">
                  <span className="flex items-center gap-2 text-[10px] font-extrabold uppercase tracking-[0.16em] text-primary-foreground/60 sm:text-xs">
                    ...
                    <ChevronDown
                      className={`size-3.5 transition-transform duration-300 ${coursePickerOpen ? "rotate-180" : ""}`}
                    />
                  </span>
                  <span className="mt-1 block line-clamp-2 break-words pr-1 text-[15px] font-extrabold leading-5 tracking-tight sm:text-xl sm:leading-6">
                    {selectedCourse?.ad ?? jt("student.chooseCourse")}
                  </span>
        ...
        <CollapsibleContent className="dropdown-enter min-w-0 overflow-hidden border-t border-border/60 bg-card p-2 sm:p-3">
          <div className="mb-2 flex items-center justify-between px-1 sm:px-2">
            <span className="text-xs font-bold text-muted-foreground">
              {jt("student.changeCourse")}
            </span>
            <Badge variant="secondary" className="rounded-lg text-[10px]">
              {jt("student.courseCount", { count: courses.length })}
            </Badge>
          </div>
          <div className="grid min-w-0 grid-cols-1 gap-1 sm:grid-cols-2 lg:grid-cols-3">
            {courses.map((course) => (
              ...
              >
                <span className="min-w-0 flex-1">
                  <span className="block line-clamp-2 break-words text-sm font-bold leading-5">
                    {course.ad}
                  </span>
                  {course.kod ? (
                ...
                <ChevronRight
                  className={`size-4 shrink-0 transition-transform duration-200 group-hover:translate-x-0.5 ${course.id === selectedCourseId ? "opacity-100" : "opacity-30"}`}
                />
              </button>
            ))}
          </div>
      ...
      <section className="min-w-0 overflow-hidden rounded-2xl border border-border/60 bg-card p-2.5 shadow-sm min-[380px]:p-3 sm:p-5">
        <div className="mb-3 flex items-center justify-between gap-3 sm:mb-4">
          <div>
            <p className="text-[10px] font-extrabold uppercase tracking-[0.14em] text-muted-foreground sm:text-xs">
              {jt("student.academicResult")}
            </p>
            <h2 className="mt-0.5 text-base font-extrabold text-foreground sm:text-xl">
              {jt("student.currentEvaluation")}
            </h2>
          </div>
          <button
            type="button"
          ...
          >
            ...
            <ChevronDown
              className={`size-4 transition-transform duration-300 ${detailsOpen ? "rotate-180" : ""}`}
            />
          </button>
        </div>

        <div className="animate-stagger grid w-full min-w-0 grid-cols-2 gap-1.5 min-[380px]:gap-2 sm:gap-3 lg:grid-cols-4">
          <ScoreMetric
          ...
          <ScoreMetric
            ...
            helper={
              <span className="text-xs font-semibold text-muted-foreground">
                {jt("student.totalResult")}
              </span>
            }
          />
          <ScoreMetric
            label={jt("common.attendance")}
            value={entries.length > 0 ? attendancePercent : null}
        ...
        <div className="mt-3 grid grid-cols-2 gap-2 xl:hidden">
          <div className="flex items-center gap-2 rounded-xl border border-border/60 bg-muted/30 px-3 py-2.5">
            ...
            <div className="min-w-0">
              <p className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground">
                {jt("common.attendance")}
              </p>
              <p className="font-data text-sm font-extrabold">
                {attended}/{entries.length}{" "}
          ...
          <div className="flex items-center gap-2 rounded-xl border border-border/60 bg-muted/30 px-3 py-2.5">
            ...
            <div className="min-w-0">
              ...
              <p
                className={`truncate text-sm font-extrabold ${attendanceStatus === "risk" ? "text-destructive" : attendanceStatus === "normal" ? "text-emerald-700 dark:text-emerald-400" : "text-foreground"}`}
              >
                {attendanceStatus === "risk"
                  ? jt("student.attendanceRisk")
                  : attendanceStatus === "normal"
                    ? jt("student.attendanceNormal")
                    : jt("common.none")}
              </p>
            </div>
          </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-xs font-extrabold text-foreground sm:text-sm">
                        {jt("student.scoreComposition")}
                      </p>
                      <p className="text-[10px] text-muted-foreground sm:text-xs">
                        {jt("student.liveFormula")}
                      </p>
                    </div>
                  </div>
                  <span className="font-data rounded-xl bg-primary px-2.5 py-1.5 text-xs font-extrabold text-primary-foreground sm:text-sm">
                    {breakdown.total}/50
                  </span>
                </div>
                <div className="grid gap-2 sm:grid-cols-2">
                  <ProgressLine
                    label={jt("common.colloquium")}
                    piece={breakdown.colloquium}
                    icon={<Layers3 className="size-3.5" />}
                  />
                  <ProgressLine
                    label={jt("common.independentWork")}
                    piece={breakdown.independent}
                    icon={<FileText className="size-3.5" />}
                  />
                  <ProgressLine
                    label={
                      breakdown.grading_type === "laboratoriya"
                        ? jt("common.laboratory")
                        : jt("common.practice")
                    }
                    piece={breakdown.daily}
                    icon={<Activity className="size-3.5" />}
                  />
                  <ProgressLine
                    label={jt("common.attendance")}
                    piece={breakdown.attendance}
                    icon={<UserCheck className="size-3.5" />}
                  />
                  {breakdown.course_work.enabled ? (
                    <ProgressLine
                      label={jt("common.courseWork")}
                      piece={breakdown.course_work}
          ...
          <Tabs defaultValue="lessons" className="w-full">
            <div className="-mx-1 overflow-x-auto px-1 pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              <TabsList className="flex h-auto w-max min-w-full justify-start rounded-xl bg-muted/70 p-1">
                <TabsTrigger
                  value="lessons"
                  className="shrink-0 rounded-lg px-3 py-2 text-xs font-extrabold sm:px-4 sm:text-sm"
                >
                  {jt("student.lessons")}
                </TabsTrigger>
                <TabsTrigger
            ...
            <TabsContent value="lessons" className="mt-3 focus-visible:outline-none sm:mt-4">
              <SectionHeading
                title={jt("student.dailyLessons")}
                subtitle={jt("student.confirmedLessons")}
            ...
            <TabsContent value="independent" className="mt-3 focus-visible:outline-none sm:mt-4">
              <SectionHeading
                title={jt("student.independentWorks")}
                subtitle={jt("student.max2Submissions")}
            ...
            {selectedCourse?.kurs_isi_var ? (
              <TabsContent value="coursework" className="mt-3 focus-visible:outline-none sm:mt-4">
                <SectionHeading
                  title={jt("student.courseWork")}
                  subtitle={jt("student.finalCourseWork")}
            ...
            <TabsContent value="colloquium" className="mt-3 focus-visible:outline-none sm:mt-4">
              <SectionHeading
                title={jt("student.colloquiums")}
                subtitle={jt("student.threeAssessments")}
          ...
          <section className="rounded-2xl border border-border/60 bg-card p-4 shadow-sm">
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-1 font-data text-2xl font-extrabold text-foreground">
                  {attended}/{entries.length}
                </p>
                <p className="mt-1 text-xs font-semibold text-muted-foreground">
                  {jt("student.absenceSummary", { absent, percent: attendancePercent })}
                </p>
              </div>
              <div
                className="relative flex size-20 shrink-0 items-center justify-center rounded-full"
                style={{
                  background: `conic-gradient(var(--primary) ${attendancePercent * 3.6}deg, var(--muted) 0deg)`,
                }}
              >
                <div className="flex size-15 items-center justify-center rounded-full bg-card font-data text-sm font-extrabold text-foreground">
                  {attendancePercent}%
                </div>
              </div>
            </div>
            <div
              className={`mt-4 flex items-center gap-2 rounded-xl border px-3 py-2.5 ${attendanceStatus === "risk" ? "border-destructive/20 bg-destructive/[0.055] text-destructive" : "border-emerald-500/20 bg-emerald-500/[0.055] text-emerald-700 dark:text-emerald-400"}`}
            >
              {attendanceStatus === "risk" ? (
      ...
      <div className="sticky bottom-3 z-20 xl:hidden">
        <Button
          type="button"
          onClick={() => setUploadOpen(true)}
          className="mx-auto flex h-11 w-fit rounded-full border border-primary-foreground/10 px-5 font-extrabold shadow-lg shadow-primary/20"
        >
...
function SectionHeading({
  ...
  return (
    <div className="mb-3 flex items-center gap-3 sm:mb-4">
      <span className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-primary/[0.075] text-primary">
        {icon}
      </span>
      <div className="min-w-0">
  ...
  if (loading) {
    return (
      <div className="space-y-2 md:hidden">
        {[0, 1, 2].map((row) => (
          <div key={row} className="skeleton-shimmer h-28 rounded-xl" />
        ))}
      </div>
    );
  }
  ...
  if (entries.length === 0) {
    return (
      <CompactEmpty
        ...
        title={jt("student.noConfirmedLessons")}
        description={jt("student.confirmedLessonHint")}
      />
    );
  }
    ...
    <>
      <div className="animate-stagger space-y-2 md:hidden">
        {entries.map((entry, index) => (
          <article
            key={entry.record.id}
            className="rounded-xl border border-border/65 bg-background p-3 transition-all duration-200 active:bg-muted/40"
          >
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0 flex-1">
                ...
                <h4 className="mt-1.5 line-clamp-2 text-sm font-extrabold leading-5 text-foreground">
                  {entry.session.movzu || jt("student.topicMissing")}
                </h4>
              </div>
              <div className="shrink-0 text-right">
                ...
                <p className="font-data mt-0.5 text-xl font-extrabold text-foreground">
                  {entry.record.grade !== null
                    ? entry.record.grade
                    : entry.record.lab_submitted
                      ? "✓"
                      : "—"}
                </p>
              </div>
          ...
          <TableBody>
            {entries.map((entry, index) => (
              <TableRow key={entry.record.id}>
                <TableCell className="text-center font-data text-xs font-bold text-muted-foreground">
                  {index + 1}
                </TableCell>
                <TableCell>
                  <p className="text-sm font-bold">{formatDate(entry.session.starts_at)}</p>
                  <p className="mt-0.5 text-[10px] text-muted-foreground">
                    {formatTime(entry.session.starts_at)}
                  </p>
                </TableCell>
                ...
                <TableCell>
                  <FileButton path={entry.record.file_url} />
                </TableCell>
                <TableCell>
                  <AttendanceBadge status={entry.record.attendance_status} />
                </TableCell>
                <TableCell>
                  {entry.record.grade !== null ? (
                    <span className="font-data text-base font-extrabold">{entry.record.grade}</span>
                  ) : entry.record.lab_submitted ? (
                    <Badge variant="secondary" className="rounded-lg text-[10px]">
                      {jt("student.labSubmitted")}
                    </Badge>
                  ) : (
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>{jt("common.topic")}</TableHead>
              <TableHead className="w-28">{jt("common.file")}</TableHead>
              <TableHead className="w-24">{jt("common.grade")}</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {displayRows.map((row, index) => (
              <TableRow key={row?.id ?? index}>
                ...
                <TableCell className="font-bold">{row?.topic || "—"}</TableCell>
                <TableCell>
                  <FileButton path={row?.file_url ?? null} />
                </TableCell>
                <TableCell className="font-data font-extrabold">{row?.grade ?? "—"}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
  ...
  return (
    <>
      <div className="md:hidden">
        <AssessmentMobileCard
          number={1}
          title={row?.topic || jt("student.courseWork")}
          date={row?.submitted_at}
          grade={row?.grade ?? null}
          file={row?.file_url ?? null}
          max="10"
    ...
    <>
      <div className="grid grid-cols-3 gap-2 md:hidden">
        {displayRows.map((row, index) => (
          <div
            key={row?.id ?? index}
            className="rounded-xl border border-border/65 bg-background p-3 text-center"
          >
            <p className="text-[10px] font-extrabold uppercase tracking-wide text-muted-foreground">
              {jt("student.ordinal", { n: index + 1 })}
            </p>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>{jt("common.date")}</TableHead>
              <TableHead className="w-32">{jt("common.grade")}</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {displayRows.map((row, index) => (
              <TableRow key={row?.id ?? index}>
                ...
                <TableCell>{formatDate(row?.tarix)}</TableCell>
                <TableCell className="font-data text-base font-extrabold">
                  {row?.grade ?? "—"}
                </TableCell>
              </TableRow>
            ))}
    ...
    <article className="rounded-xl border border-border/65 bg-background p-3">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <h4 className="mt-1 line-clamp-2 text-sm font-extrabold leading-5 text-foreground">
            {title}
          </h4>
        </div>
        <div className="shrink-0 text-right">
...
function CompactEmpty({
  ...
  return (
    <div className="flex min-h-52 flex-col items-center justify-center rounded-xl border border-dashed border-border bg-muted/20 px-6 py-8 text-center">
      <span className="flex size-11 items-center justify-center rounded-2xl bg-primary/[0.075] text-primary">
        {icon}
      </span>
...
function SubmissionDialog({
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      try {
        if (kind === "independent") {
          const usedTopic = independentWorks.some(
            (row) => row.topic_id === topicId && (row.file_url || row.submitted_at),
          );
          if (usedTopic) throw new Error(jt("student.topicAlreadySubmitted"));
          const free = independentWorks.find(
            (row) => !row.file_url && !row.submitted_at && row.status === "gozleyir",
          );
          ...
          if (courseWork.file_url || courseWork.submitted_at || courseWork.status !== "gozleyir")
            throw new Error(jt("student.alreadySubmitted"));
          const { error } = await supabase
            .from("course_work_assessments")
            .update({
              topic_id: topicId,
          ...
          if (course.qiymetlendirme_novu !== "laboratoriya")
            throw new Error(jt("student.notLaboratoryCourse"));
          const topicType = normalizeText(selectedTopic.dars_novu);
          const candidates = sessions.filter(
            (session) => !session.is_confirmed && normalizeText(session.dars_novu).includes("lab"),
          );
          const session =
            candidates.find((row) => row.topic_id === topicId) ??
            candidates.find(
              (row) =>
                row.lesson_date === selectedTopic.tarix &&
                (topicType.includes("lab") || normalizeText(row.dars_novu).includes(topicType)),
            ) ??
          ...
          if (record.file_url || record.lab_submitted)
            throw new Error(jt("student.alreadySubmitted"));
          const { error } = await supabase
            .from("lesson_student_records")
            .update({ file_url: path, lab_submitted: true })
            .eq("id", record.id)
            .eq("student_id", userId);
          if (error) throw error;
        }
  ...
  const independentFull =
    independentWorks.filter((row) => row.file_url || row.submitted_at).length >= 2;
  const courseWorkFull = Boolean(
    courseWork &&
    (courseWork.file_url || courseWork.submitted_at || courseWork.status !== "gozleyir"),
  );
      ...
      <DialogContent className="max-h-[92dvh] max-w-xl overflow-y-auto rounded-2xl border-border bg-card p-0 sm:p-0 sm:rounded-3xl">
        <div className="border-b border-border/60 bg-primary/[0.045] p-5 sm:p-6">
          <DialogHeader>
            ...
            <DialogTitle className="mt-3 text-xl font-extrabold sm:text-2xl">
              {jt("student.submitDialogTitle")}
            </DialogTitle>
          </DialogHeader>
          <p className="mt-1 text-xs leading-5 text-muted-foreground sm:text-sm">
            {jt("student.submitDialogDescription", { course: course.ad })}
          </p>
        </div>
        ...
        <div className="space-y-4 p-5 sm:p-6">
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-1.5">
              <label className="text-xs font-extrabold text-foreground">
                {jt("student.assignmentType")}
              </label>
              <Select
                value={kind}
                onValueChange={(value) => {
                  setKind(value as SubmissionType);
                  setTopicId("");
                }}
              >
                <SelectTrigger className="h-12 rounded-xl bg-muted/50 px-3">
                  <SelectValue placeholder={jt("student.chooseType")} />
                </SelectTrigger>
                <SelectContent>
                  {course.qiymetlendirme_novu === "laboratoriya" ? (
                    <SelectItem value="lab">{jt("common.laboratory")}</SelectItem>
                  ) : null}
                  <SelectItem value="independent" disabled={independentFull}>
                    {jt("common.independentWork")}
                    {independentFull ? ` — ${jt("student.limitFull")}` : ""}
                  </SelectItem>
                  {course.kurs_isi_var ? (
                    <SelectItem value="coursework" disabled={courseWorkFull}>
                      {jt("common.courseWork")}
                      {courseWorkFull ? ` — ${jt("student.submitted")}` : ""}
                    </SelectItem>
                  ) : null}
                </SelectContent>
              </Select>
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                disabled={!kind || filteredTopics.length === 0}
              >
                <SelectTrigger className="h-12 rounded-xl bg-muted/50 px-3">
                  <SelectValue
                    placeholder={
                      filteredTopics.length === 0
                        ? jt("student.topicNotAdded")
                        : jt("student.chooseTopic")
                    }
                  />
                </SelectTrigger>
                <SelectContent>
          ...
          >
            <span
              className={`flex size-12 items-center justify-center rounded-2xl transition-transform duration-300 group-hover:-translate-y-0.5 ${file ? "bg-emerald-500/10 text-emerald-600" : "bg-primary/10 text-primary"}`}
            >
              {file ? <CheckCircle2 className="size-6" /> : <CloudUpload className="size-6" />}
            </span>
            <p className="mt-3 text-sm font-extrabold text-foreground">
              {file ? jt("student.fileSelected") : jt("student.dropFile")}
            </p>
            <p className="mt-1 max-w-sm truncate text-xs text-muted-foreground">
              {file ? file.name : jt("student.maxFile25")}
            </p>
            <input
              type="file"
              className="hidden"
        ...
        <DialogFooter className="gap-2 border-t border-border/60 bg-muted/20 p-4 sm:p-5">
          ...
          <Button
            ...
            disabled={mutation.isPending || !kind || !topicId || !file}
          >
            {mutation.isPending ? (
