...
import { StudentDashboard, StudentHomeSkeleton } from "@/components/dashboard/StudentDashboard";
...
function EvSehifesi() {
  ...
  if (error || !userId)
    return (
      <div className="my-10 flex flex-1 flex-col items-center justify-center rounded-3xl bg-card p-8 text-center shadow-sm">
        <AlertCircle className="mb-3 size-12 text-destructive" />
        <h3 className="text-lg font-bold text-foreground">
          {t("common.loadError", "Məlumat yüklənə bilmədi")}
        </h3>
        <p className="mt-1 max-w-xs text-sm text-muted-foreground">
          {t(
            "common.loadErrorDescription",
            "Hesabınıza aid məlumatlar çəkilərkən xəta baş verdi. Zəhmət olmasa yenidən cəhd edin.",
          )}
        </p>
        <button
          onClick={() => refetch()}
          className="mt-4 inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-primary px-5 text-sm font-bold text-primary-foreground transition hover:opacity-90"
        >
  ...
  switch (primaryRole) {
    ...
    case "dekan":
      return <DeanDashboard userId={userId} />;
    case "muellim":
    ...
    default:
      return (
        <StudentDashboardLocaleBridge>
          ...
          <StudentAnnouncementsHome userId={userId} />
        </StudentDashboardLocaleBridge>
      );
  }
}
