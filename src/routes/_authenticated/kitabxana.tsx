...
export const Route = createFileRoute("/_authenticated/kitabxana")({
  head: () => ({
    meta: [
      { title: "ATU Portal" },
  ...
  component: KitabxanaSehifesi,
});

function KitabxanaSehifesi() {
  const { roles } = useUserRoles();
  const { t } = usePageI18n();
  const inzibatciMi = roles.includes("admin") || roles.includes("dekan");
  const [axtaris, setAxtaris] = useState("");
  const [kateqoriya, setKateqoriya] = useState<string | null>(null);
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const sona = basdan + SEHIFE_OLCUSU - 1;
      let sorgu = supabase
        .from("library_books")
        .select("*", { count: "exact" })
        ...
        .range(basdan, sona);
      if (axtarisAktivdir) {
        const deyer = axtaris.trim().replace(/[%,]/g, "");
        sorgu = sorgu.or(`ad.ilike.%${deyer}%,muellif.ilike.%${deyer}%`);
      } else if (kateqoriya) sorgu = sorgu.eq("kateqoriya", kateqoriya);
      const { data: netice, error, count } = await sorgu;
      if (error) throw error;
      return { kitablar: (netice ?? []) as LibraryBook[], sayi: count ?? 0 };
    },
    enabled: siyahiRejimi,
  });
  const kitablar = data?.kitablar ?? [];
  const sehifeSayi = Math.max(1, Math.ceil((data?.sayi ?? 0) / SEHIFE_OLCUSU));
  function kateqoriyaSec(ad: string) {
    setKateqoriya(ad);
    setSehife(1);
  }
  function geriQayit() {
  ...
  function kitabAc(kitab: LibraryBook) {
    setSeçilmişKitab(kitab);
    setModalAçıq(true);
  }
  return (
    <div className="flex flex-1 flex-col gap-4 pb-20 md:gap-5 md:pb-0">
      {siyahiRejimi ? (
        <PageHeader
          ...
          onGeri={geriQayit}
        >
          {inzibatciMi ? <AddBookForm /> : null}
        </PageHeader>
      ) : null}
      {!siyahiRejimi ? (
        <LibraryHero
          axtaris={axtaris}
          onAxtarisDeyis={setAxtaris}
          onKateqoriyaSec={kateqoriyaSec}
        />
      ) : null}
      {siyahiRejimi ? (
        <div
          key={`${kateqoriya ?? "search"}-${axtaris}`}
          className="library-premium-section library-category-results-panel rounded-[28px] p-4 sm:p-6"
        >
          <BookResultsGrid
            kitablar={kitablar}
            yuklenir={isLoading}
            ...
            onKitabSec={kitabAc}
            boşMesaj={axtarisAktivdir ? t("library.emptySearch") : t("library.emptyCategory")}
          />
        </div>
      ) : (
        <>
          ...
          <CategoryGrid onKateqoriyaSec={kateqoriyaSec} />
          <RecentBooks onKitabSec={kitabAc} />
        </>
      )}
